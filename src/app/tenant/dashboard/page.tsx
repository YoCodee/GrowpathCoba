...
const TenantSidebar = ({ active, setActive, profileName, signOut }) => {
// ... (Kode TenantSidebar tetap sama) ...
    const navItems = [
    ...
    return (
        <div className="flex flex-col w-64 bg-gray-900 text-white min-h-screen p-4 shadow-xl">
                ...
                {navItems.map((item) => (
                    <button
                        ...
                        className={`flex items-center w-full p-3 rounded-lg transition-colors 
                            ${active === item.path ? 'bg-blue-600 font-semibold' : 'hover:bg-gray-700'}`
                        }
                    >
            ...
            <div className="mt-auto pt-4 border-t border-gray-700">
                <p className="text-sm font-medium mb-2 text-gray-400">Selamat datang,</p>
                <p className="font-semibold truncate">{profileName || 'Tenant'}</p>
                <button
                    onClick={signOut}
                    className="mt-3 flex items-center w-full p-2 text-sm rounded-lg bg-red-600 hover:bg-red-700 transition-colors"
                >
...
const DashboardContent = ({ summary, loading }: { summary: DashboardSummary, loading: boolean }) => {
    ...
    if (loading) {
        ...
        return (
            <div className="p-8 w-full">
                <h1 className="text-3xl font-bold mb-6 text-gray-800">Ringkasan Cashflow Anda</h1>
    ...
    const stats = [
        ...
        { label: "Pengeluaran Hari Ini", value: summary.totalExpense, color: "red" },
        { label: "Saldo Bersih", value: summary.netCash, color: "blue" },
        { label: "Total Transaksi", value: summary.totalSalesCount, color: "yellow", isCurrency: false },
    ];
    ...
    return (
        <div className="p-8 w-full">
            <h1 className="text-3xl font-bold mb-6 text-gray-800">Ringkasan Cashflow Hari Ini 📅</h1>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat) => (
                    <div key={stat.label} className={`bg-white p-6 rounded-xl shadow-lg border-l-4 border-${stat.color}-500 transition-shadow hover:shadow-xl`}>
                        ...
                        <p className="text-xs text-gray-400 mt-2">Data dihitung dari transaksi hari ini.</p>
                    </div>
                ))}
            </div>

            <h2 className="text-2xl font-semibold mt-10 mb-4 text-gray-800">Aktivitas Cepat</h2>
...
const ProductManagement = ({ products, fetchProducts, userId, tenantId }: { products: Product[], fetchProducts: () => void, userId: string | undefined, tenantId: number | undefined }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            setMessage(`Produk berhasil ${editingProduct ? 'diperbarui' : 'ditambahkan'}!`);
            setName('');
            setPrice('');
    ...
    if (!tenantId && !loading) return <div className="p-8 text-red-600">Gagal memuat Tenant ID.</div>;


    return (
        <div className="p-8 w-full">
            <h1 className="text-3xl font-bold mb-6 text-gray-800">Kelola Produk ({products.length})</h1>
            {message && (
                <div className={`p-3 rounded mb-4 text-white ${message.includes('gagal') ? 'bg-red-500' : 'bg-green-500'}`}>
                    {message}
                </div>
            )}
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
                <h2 className="text-xl font-semibold mb-4">{editingProduct ? 'Edit Produk' : 'Tambah Produk Baru'}</h2>
                <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <div className="flex space-x-2 w-full md:w-auto">
                        ...
                        {editingProduct && (
                            <button
                                type="button"
                                onClick={() => { setEditingProduct(null); setName(''); setPrice(''); }}
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg">
                ...
                {products.length === 0 ? (
                    <p className="text-gray-500">Belum ada produk. Silakan tambahkan satu di atas.</p>
                ) : (
                    <ul className="space-y-3">
                        {products.map((product) => (
                            <li key={product.id} className="flex justify-between items-center p-3 border-b last:border-b-0 hover:bg-gray-50 rounded-md">
                                <div>
...
const QuickCashflowEntry = ({ products, tenantId, onSaleRecorded }: { products: Product[], tenantId: number | undefined, onSaleRecorded: () => void }) => {
    ...
    const handleCheckout = async () => {
        ...
        const { data: saleData, error: saleError } = await supabase
            ...
            .insert({ 
                ...
                notes: `Penjualan cepat: ${cart.map(item => item.product.name).join(', ')}`
            })
            .select('id')
            .single();
        ...
        if (itemsError || transactionError) {
            setMessage(`Penjualan tercatat, tapi gagal mencatat detail/cashflow: ${itemsError?.message || transactionError?.message}`);
        } else {
            setMessage('Penjualan **BERHASIL** dicatat! Pemasukan telah ditambahkan ke laporan.');
            setCart([]);
    ...
    return (
        <div className="p-8 w-full">
            <h1 className="text-3xl font-bold mb-6 text-gray-800">Jual Cepat (Quick Sale) ⚡</h1>
            {message && (
                <div className={`p-3 rounded mb-4 text-white ${message.includes('BERHASIL') ? 'bg-green-500' : 'bg-red-500'}`} dangerouslySetInnerHTML={{ __html: message }}>
                </div>
            )}
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-2/3 bg-white p-6 rounded-xl shadow-lg">
                    <h2 className="text-xl font-semibold mb-4 border-b pb-2">Pilih Produk (Tap untuk tambah)</h2>
                    {products.length === 0 ? (
                        <p className="text-gray-500 p-4 bg-yellow-50 rounded">
                            ⚠️ Belum ada produk. Silakan tambahkan di tab "Kelola Produk" terlebih dahulu.
                        </p>
                    ) : (
                ...
                <div className="lg:w-1/3 bg-white p-6 rounded-xl shadow-lg border-2 border-blue-500 sticky top-4 h-fit">
                    <h2 className="text-xl font-semibold mb-4 border-b pb-2">Keranjang Belanja ({cart.length} item)</h2>
                    
                    {cart.length === 0 ? (
                        <p className="text-gray-500 italic">Keranjang kosong. Tambahkan produk!</p>
                    ) : (
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (newQuantity <= 0) {
                                                    ...
                                                    setCart(prev => prev.map(i => i.product.id === item.product.id ? { ...i, quantity: newQuantity } : i));
                                                }
                                            }}
                                            min="1"
...
const ExpenseEntryWithCallback = ({ tenantId, onExpenseRecorded }: { tenantId: number | undefined, onExpenseRecorded: () => void }) => {
    ...
    const handleExpenseSubmit = async (e: React.FormEvent) => {
        ...
        if (!tenantId || parseFloat(amount) <= 0) {
            setMessage('Error: ID Tenant tidak ditemukan atau jumlah biaya tidak valid.');
            return;
        }
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-lg">
            <h2 className="text-xl font-semibold mb-4 border-b pb-2">Catat Biaya (Pengeluaran) 💸</h2>
            {message && (
                <div className={`p-3 rounded mb-4 text-white ${message.includes('BERHASIL') ? 'bg-green-500' : 'bg-red-500'}`} dangerouslySetInnerHTML={{ __html: message }}>
                </div>
            )}
            <form onSubmit={handleExpenseSubmit} className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700">Deskripsi Biaya</label>
                    <input
                        type="text"
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700">Jumlah Biaya (Rp)</label>
                    <input
                        type="number"
                        placeholder="150000"
                        value={amount}
...
const ReportsContent = ({ tenantId, onReportUpdate }: { tenantId: number | undefined, onReportUpdate: () => void }) => {
    ...
    if (!tenantId) return <div className="p-8 text-red-600">Gagal memuat Tenant ID.</div>;
    ...
    return (
        <div className="p-8 w-full">
            <h1 className="text-3xl font-bold mb-6 text-gray-800">Laporan Cashflow 📊</h1>

            {/* Bagian Catat Biaya (Expense) */}
            <div className="mb-8">
                <ExpenseEntryWithCallback tenantId={tenantId} onExpenseRecorded={handleExpenseRecorded} /> 
            </div>
            
            {/* Ringkasan Cashflow */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-green-100 p-6 rounded-xl shadow border-l-4 border-green-500">
                    <p className="text-sm font-semibold text-green-700">Total Pemasukan (Global)</p>
                    <p className="text-2xl font-bold mt-1">Rp {summary.income.toLocaleString('id-ID')}</p>
                </div>
                <div className="bg-red-100 p-6 rounded-xl shadow border-l-4 border-red-500">
                    <p className="text-sm font-semibold text-red-700">Total Pengeluaran (Global)</p>
                    <p className="text-2xl font-bold mt-1">Rp {summary.expense.toLocaleString('id-ID')}</p>
                </div>
                <div className="bg-blue-100 p-6 rounded-xl shadow border-l-4 border-blue-500">
                    <p className="text-sm font-semibold text-blue-700">Saldo Bersih (Net Cash)</p>
                    <p className="text-2xl font-bold mt-1">Rp {summary.net.toLocaleString('id-ID')}</p>
                </div>
            </div>

            {/* Daftar Transaksi dengan Pagination */}
            <div className="bg-white p-6 rounded-xl shadow-lg">
                <h2 className="text-xl font-semibold mb-4 border-b pb-2">Riwayat Transaksi Terakhir (Halaman {currentPage} dari {totalPages})</h2>
                
                {loading ? (
                    <div className="text-center p-10 text-gray-500">Memuat riwayat transaksi...</div>
                    ...
                    <>
                        <div className="max-h-[50vh] overflow-y-auto mb-4">
                            <ul className="space-y-3">
                                {transactions.map((t) => (
                                    <li key={t.id} className="flex justify-between items-center p-3 border-b last:border-b-0 hover:bg-gray-50 rounded-md">
                                        <div>
                                            <p className="font-semibold">{t.description}</p>
                                            <p className="text-xs text-gray-500">{new Date(t.transaction_date).toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
...
export default function TenantDashboardPage() {
    ...
    const ensureTenantRowExists = async (authUserId: string, storeName: string) => {
        const { data } = await supabase.from('tenants').select('id').eq('user_id', authUserId).single();
    ...
    const fetchProducts = async (currentTenantId: number) => {
        setDataLoading(true);
        const { data, error } = await supabase.from('products').select('*').eq('tenant_id', currentTenantId); 
        if (error) {
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'dashboard':
                ...
                return <DashboardContent summary={dashboardData} loading={dashboardLoading} />;
            case 'products':
            ...
            case 'reports':
                ...
                return <ReportsContent tenantId={tenantId} onReportUpdate={handleDataRefresh} />;
            default:
                return <DashboardContent summary={dashboardData} loading={dashboardLoading} />;
        }
    }
